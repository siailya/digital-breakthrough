...
class FastTextWrapper:
    ...
    def predict(self, text: str, n: int=10):
        ...
        data_top_k_inds[data_top_k_inds["liter"].notna()]["liter"] = data_top_k_inds[data_top_k_inds["liter"].notna()]["liter"].apply(lambda x: x.lower())
        features = self.ext.get_features(text)
        if features["structure"] is not None:
            data_top_k_inds["structure"] = data_top_k_inds["full_address"].apply(lambda x: self.ext.get_features(x)['structure'])
        ...
        if len(data_top_k_inds) != 0:
            ...
            return [(row["id"], row["sim"]) for ind, row in data_top_k_inds.iterrows()]
        
        return top_k.sort(key=lambda a: a[1])
